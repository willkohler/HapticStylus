...
public class Hand : MonoBehaviour
{
    ...
    private string IP_address = "192.168.0.120";
    ...
    // Function to update motor position if hand is in contact with the surface
    // This updates the position gradually. It moves 3.5 mm upon surface contact and additional mm value matching additional dist below surface
    // This scheme doesn't use levels below the coutner as originally done
    private double CalcMotorPositionBelowSurface(float ycoord)
    {
        var motorPositionOnContact = 3.5; // in mm

        // Calculate number of levels the hand is below the counter
        var distBelowCounter = Math.Abs(ycoord) - 4.3;

        // Calculate position motor should be set to
        var motorPos = motorPositionOnContact + (distBelowCounter * 1);
